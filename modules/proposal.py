...
DK1        = RGBColor(0x0E, 0x28, 0x41)  # primary navy
DK2        = RGBColor(0x0E, 0x28, 0x41)  # secondary dark (unified with DK1)
...
def _add_hedge_chart(sl, left, top, width, height,
    ...
    if proj_df is not None and "PPA Cost ($)" in proj_df.columns and len(proj_df) >= term:
        ...
        # No PPA rate set — use savings percentage backsolve as fallback
        ppa_total_list = []
        sf = savings_pct / 100.0
        for yr_idx in range(term):
            r = proj_df_original.iloc[yr_idx]
        ...
        # Fallback: single PPA rate + escalator (no regime switch awareness)
...
def _slide_key_commercial_terms(
    ...
    _takeaway(
        sl,
        "Predictable savings versus the utility bill with zero customer capex, "
...
def _slide_year1(prs, pg, total, ex, result, tariff, proj_df=None,
    ...
    _txt(sl, ML, Inches(5.80), CW, Inches(0.30),
         text="Savings grow each year as utility rates escalate while the PPA is priced each year to keep the customer saving versus the utility bill.",
         sz=Pt(9), color=GRAY50, align=PP_ALIGN.CENTER)
...
def _slide_rate_hedge(prs, pg, total, ex, utility, ppa, ppa_esc, term,
    ...
    _accent_rule(sl)
    _action_title(sl,
        "Predictable PPA pricing hedges against rising utility costs",
        exhibit=ex)
    _subtitle(sl, f"{utility}  |  Rate certainty as a strategic advantage")

    _takeaway(sl,
        f"{'The PPA' if ppa else 'Solar'} keeps the customer's energy cost predictable and below the utility bill for {term} years. "
    ...
    considerations = [
        ...
        (f"PPA rate is set each year to hold the customer's target savings for the full {term}-year term"
         if ppa else
...
def _savings_matrix_table(sl, l, t, w, col_ws, group_hdrs, sub_hdrs, rows,
                          bold_last=False, sz=Pt(7)):
    """Table with two-row header: merged color-coded group headers + sub-headers.

    group_hdrs: list of (label, span, bg_color) for row 0 merged cells
    sub_hdrs:   list of str for row 1 (same length as columns)
    """
    ...
    col_idx = 0
    for label, span, bg in group_hdrs:
        if span > 1:
            tbl.cell(0, col_idx).merge(tbl.cell(0, col_idx + span - 1))
        c = tbl.cell(0, col_idx)
        c.text = label
        _cfmt(c, bold=True, sz=Pt(8), color=WHITE, bg=bg, al=PP_ALIGN.CENTER)
        col_idx += span

    # Row 1 — sub-headers
    for j, h in enumerate(sub_hdrs):
        c = tbl.cell(1, j)
        c.text = h
        _cfmt(c, bold=True, sz=sz, color=WHITE, bg=TBL_HDR, al=PP_ALIGN.CENTER)
...
def _slide_savings_matrix(prs, pg, total, ex, proj_df,
    ...
    group_hdrs = [
        ("", 1, TBL_HDR),                       # Year
        ("", 1, TBL_HDR),                       # NEM
        ("", 1, TBL_HDR),                       # Utility Savings
        ("5% Customer Savings", 2, ACCENT1),    # green
        ("10% Customer Savings", 2, ACCENT3),   # teal
        ("15% Customer Savings", 2, ACCENT4),   # blue
    ]
...
def _slide_label_for_primary(
    nem_regime_1, nem_regime_2, ppa_rate, ppa_rate_r2,
) -> str:
...
def generate_proposal_pptx(
    *,
    ...
    if has_p:
        pg += 1; ex += 1
        _slide_savings_matrix(prs, pg, total, ex, _matrix_df,
                              nem_regime_1=nem_regime_1,
                              nem_regime_2=nem_regime_2,
                              num_years_1=num_years_1,
                              term_years=term_years,
    ...
    if comparison_ppas:
        ...
        _slide_alternatives_considered(
            prs, pg, total,
            primary_label=f"{_slide_label_for_primary(nem_regime_1, nem_regime_2, ppa_rate, ppa_rate_regime_2)}",
